...
def hours_spent(from_date, to_date):
    ...
    first_day = min((last_hour - start_date.hour) if start_date.hour < last_hour else 0, hrs_a_day)
    last_day = min((end_date.hour - first_hour) if end_date.hour > first_hour else 0, hrs_a_day)
    ...
    if start_date.date() == end_date.date():
        ...
        result = first_day + last_day + (len(weekday_numbers) * hrs_a_day)
    return result
